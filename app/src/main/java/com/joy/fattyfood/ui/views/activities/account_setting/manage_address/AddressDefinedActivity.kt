...
import com.joy.fattyfood.domain.viewstates.AddressViewState
...
import com.joy.fattyfood.databinding.ActivityAddressDefinedBinding
...
import com.joy.fattyfood.domain.view_model.AddressViewModel
        ...
        _binding.btnConfirmLocation.setOnClickListener {
            ...
            if (status != "" && currentAddress != "") {
                if(updateStatus == "update") PreferenceUtils.readUserVO().customer_id?.let { it1 ->
                    viewModel.updateCurrentAddress(customerAddress.customer_address_id,
                        it1,locations.latitude, locations.longitude, currentAddress,PreferenceUtils.readUserVO().customer_phone, building, status)
                }
                else if (_binding.rbtDefaultAddress.isChecked) {
                    viewModel.setUpDefaultAddress(customerAddress.customer_address_id)
                }
...
//        }
        ...
        println("DDDDDDDDDDDd ${convertToCurrentAddress(LatLng(locations.latitude, locations.longitude))}")
    }
