...
class OrderHistoryViewHolder(var binding : ItemOrderHistoriesBinding, var callback : (MyOrderHistoryResponse.Data.Data, str : String, pos : Int) -> Unit) : BaseViewHolder<MyOrderHistoryResponse.Data.Data>(binding.root)  {

    override fun setData(data: MyOrderHistoryResponse.Data.Data, position: Int) {


        binding.tvOrderTime.text = data.order_time
        binding.tvOrderId.text = ContextCompat.getString(binding.root.context,R.string.order_id).plus(data.order_id)

        binding.tvItemQty.text = "${data.item_count}".plus(if (data.item_count == 1) " Item" else " Items")



        if (data.order_type == "food") {
            binding.tvRestNameFromAddress.text = data.restaurant_name
            binding.tvPriceOrToAddress.text = "${data.total} ${if (data.currency_id == 1) "MMK" else "¥"}"
            ...
            when(data.order_status_id) {
                1 -> {
                    binding.tvOrderActionsStatus.text = ContextCompat.getString(binding.root.context, R.string.str_cancel)
                ...
                9 -> {
                    ...
                    binding.tvOrderStatusMsg.text = data.order_status
                    binding.tvOrderStatusMsg.setTextColor(ContextCompat.getColor(binding.root.context,R.color.order_status_error))
                }
                18 -> {
                    binding.tvOrderActionsStatus.gone()
                    binding.tvOrderStatusMsg.text = data.order_status
            ...
            when(data.order_status_id){
                ...
                12 -> {
                    binding.tvOrderActionsStatus.show()
                    binding.tvOrderActionsStatus.text = binding.root.context.getString(R.string.str_track)
                    binding.tvOrderActionsStatus.setBackgroundResource(R.drawable.negative_corner_track)
                    binding.tvOrderStatusMsg.text = data.order_status
                    binding.ivOrderStatusIcon.setImageResource(R.drawable.ic_order_processing_20dp)
                    binding.tvOrderStatusMsg.setTextColor(ContextCompat.getColor(binding.root.context,R.color.order_processing_color))
                    actions = "track"
                }
                13 -> {
                    binding.tvOrderActionsStatus.show()
                ...
                15 -> {
                    ...
                    binding.tvOrderStatusMsg.text = data.order_status
                    binding.ivOrderStatusIcon.setImageResource(R.drawable.ic_order_success_20dp)
                    binding.tvOrderStatusMsg.setTextColor(ContextCompat.getColor(binding.root.context,R.color.success200))
                }
                16 -> {
                    binding.tvOrderStatusMsg.text = data.order_status
                }
