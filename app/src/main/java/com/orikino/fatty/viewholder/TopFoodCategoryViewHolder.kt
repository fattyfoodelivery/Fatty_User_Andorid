...
class TopFoodCategoryViewHolder(var binding : ItemTopFoodCategoryBinding, val imageUrlProvider: ImageUrlProvider,var callback : (CategoryVO) -> Unit) : BaseViewHolder<CategoryVO>(binding.root)  {

    override fun setData(data: CategoryVO, position: Int) {
        binding.tvFoodName.text = data.toDefaultCategoryName()
        binding.foodImage.load(imageUrlProvider.get("/category/"+data.restaurant_category_image)) {
