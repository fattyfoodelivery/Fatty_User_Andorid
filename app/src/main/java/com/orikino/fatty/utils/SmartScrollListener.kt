 ...
 */
 open class SmartScrollListener(
    ...
    companion object {
        const val FLAG_VERTICALLY = 0
        const val FLAG_HORIZONTAL = 1
    }

    protected var isListEndReached = false
    protected var newItemAlert: View? = null
    ...
    override fun onScrollStateChanged(recyclerView: RecyclerView, scrollState: Int) {
        ...
        if (mFlag == FLAG_VERTICALLY && !recyclerView.canScrollVertically(1) && !isListEndReached && scrollState ==RecyclerView.SCROLL_STATE_IDLE ) {
        ...
        if (mFlag == FLAG_HORIZONTAL && !recyclerView.canScrollHorizontally(1) && !isListEndReached && scrollState ==RecyclerView.SCROLL_STATE_IDLE ) {
            isListEndReached = true
            limit = recyclerView.layoutManager!!.itemCount
            mSmartScrollListener.onListEndReach()
        }
